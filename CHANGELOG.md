...
- Established default agent dispatch for Agent Orchestrators with a lawful P14
  ...
  signal requirement in restoration handouts and rotation prompts (with the
  trace filename convention `00_handout_agent.md` vs `00_handout_readonly.md`);
  and added an explicit pin-time presentation and envelope review hook in
  `UPDATING.md` with an illustrative project-owned presentation capsule in
  `INTEGRATION.md`. `AP.md` remains the sole semantic owner. No 4th role, no emoji-as-AP-field, no mechanical
...
- Added risk-routed orchestration and implementation planning, healthy
  ...
  fresh independent acceptance, repeated-blocker escalation,
  failure-preserving automation and privilege rules, expanded communication and
  surface routing, and human-governed collaboration that preserves meaningful
  Cooperator decisions without deterministic-step microapproval; recorded by
  ADR-0011 with semantic scenario coverage.
