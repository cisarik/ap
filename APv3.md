# Analytic Programming Protocol (Version 3)

> **Active protocol for this source repository.** This is AP version 3.
> It is a single-Worker Coordinator Protocol with intentional Worker instance rotation.
> It supersedes AP version 1 (now a redirect in `AP.md`) and AP version 2 (`APv2.md`, retained as superseded experimental reference).
> Consuming projects copy this file to `AP.md`. See [VERSIONING.md](VERSIONING.md) and [ADOPTION.md](ADOPTION.md).
...
An **execution client** is the host tool or environment running the agent. An **agent implementation** is the concrete agent system fulfilling capabilities. A **model** is the language model used for generation. A **model provider** is the organization or service hosting that model. A specific coding agent — whether IDE-integrated, command-line, or otherwise — may be an execution client or agent implementation; it is not the WORKER role itself.
...
Tool availability, an enabled browser setting, or a previously successful automation run does not grant task authority.
...
- inspect DOM, network responses, storage, rendered state, screenshots, or browser logs;
...
- whether navigation, clicks, typing, DOM reads, JavaScript execution, request interception, storage inspection, screenshots, downloads, or browser logs are allowed;
- whether external network traffic is allowed;
- permitted test data and account state;
- temporary-script and evidence retention rules;
- required cleanup.
...
Platform-specific adapters belong in project-specific bootstrap or environment documentation, not as reusable protocol requirements.

A missing browser adapter SHOULD produce honest missing evidence or `BLOCKED` status when browser evidence is mandatory; it MUST NOT justify unsafe workarounds.
...
User-visible work is not fully accepted by source inspection or automated tests alone when rendered behavior, physical interaction, media playback, native UI, or device state matters.

After implementation verification, the ORCHESTRATOR SHOULD prepare a numbered checklist of independently observable outcomes. Each item SHOULD test one clear behavior where practical.
...
The `+` marker adds evidence or context. It does not silently change the item status.

Screenshots, video, logs, physical observations, or other evidence SHOULD identify the relevant item number and state.

The ORCHESTRATOR MUST classify each response as accepted behavior, concrete defect, missing evidence, new product decision, or adjacent scope. Concrete defects MAY become bounded correction tasks. New ideas do not automatically expand the current Worker task.

Rendered acceptance evidence MUST be distinguished from Worker-observed tests and static source claims.
...
## Relationship to AP v1 and AP v2

AP version 3 supersedes AP version 1 and AP version 2.

- `AP.md` in this source repository is now a redirect to this document (see `AP.md`).
- `APv2.md` is retained as a superseded experimental reference. Its multi-Worker topology material was experimental and theoretical only; this repository operates a single-Worker model with intentional Worker instance rotation.
- Consuming projects copy this file (`APv3.md`) to `AP.md` as their active protocol.
- Git history preserves v1 and v2 text for traceability.

See [ADR-0003](docs/adr/0003-apv3-protocol-generation.md) for the generation decision and [VERSIONING.md](VERSIONING.md) for the supersession semantics.
